...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        {
            let id_column = get_column_string(&(id.to_string()), 12);
            let name_column = get_column_string(&epic.name, 34);
            let status_column = get_column_string(&epic.status.to_string(), 18);
