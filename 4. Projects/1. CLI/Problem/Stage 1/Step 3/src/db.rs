...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        if db_state.epics.contains_key(&epic_id) {
            return Err(anyhow!(
                "Could not insert epic {}, ID {} already exists",
                epic.name,
                epic_id
            ));
        } else {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        let mut db_state = self.read_db()?;
        let epic: &mut Epic = db_state
            .epics
            .get_mut(&epic_id)
            .ok_or(anyhow!("Could not find epic with ID: {}", epic_id))?;
        let story_id = db_state.last_item_id + 1;
        if epic.stories.contains(&story_id) {
            Err(anyhow!(
                "Found unexpected duplicate for Story ID: {}",
                story_id
            ))
        } else {
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        // remove epic from epics
        // remove epic stories from stories
        // keep last_item_id untouched
        let mut db_state = self.read_db()?;
        match db_state.epics.remove(&epic_id) {
            Some(epic) => {
                for story_id in epic.stories {
                    let _ = db_state.stories.remove(&story_id);
                }
                self.write_db(&db_state)?;
                Ok(())
            }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        match db_state.stories.remove(&story_id) {
            Some(_) => match db_state.epics.get_mut(&epic_id) {
                Some(epic) => {
                    if let Some(pos) = epic.stories.iter().position(|&s| s == story_id) {
                        epic.stories.remove(pos);
                        self.write_db(&db_state)?;
                        Ok(())
                    } else {
                        Err(anyhow!(
                            "Story ID {}, not found in Epic with ID {}",
                            story_id,
                            epic_id
                        ))
                    }
            ...
            None => Err(anyhow!("Story with ID {} not found", story_id)),
        }
    }
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        ...
        match db_state.epics.get_mut(&epic_id) {
            ...
            None => Err(anyhow!("No Epic with ID {} found", epic_id)),
        }
    }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        match db_state.stories.get_mut(&story_id) {
            ...
            None => Err(anyhow!("No Story with ID {} found", story_id)),
        }
    }
}
